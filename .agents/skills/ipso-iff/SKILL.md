...
Before dispatching to the detective phase, run `.agents/skills/ipso-iff/scripts/load_actors.py`
...
Run `.agents/skills/ipso-iff/scripts/generate_session_id.py` to generate a fresh `session_id`
...
`<session folder>/dataset.json` (see `.agents/ipso-annotator.md` for the schema).
